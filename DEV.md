...
**优化指令的编辑契约（v0.12.5 定稿）**：只有一条规则 —— **当前内置文案 = 未自定义；
其余一律按用户的自定义指令原样保存，插件不干预**。不满意就点「恢复默认指令」。

配套的两点：

1. 编辑器**每次展开都从 Host 重新拉取**（保留未保存的草稿，只在草稿等于基准文本时才
   覆盖）。所以设置页跨版本开着也不会把过期文本存回去 —— 问题在源头解决，不需要在
   服务端猜"这段文本是不是旧版内置"。
2. 为什么不去猜：v0.12.4 试过两版推测机制，都失败了。第一版只比对当前版本（太窄，
   正是它要修的问题）；第二版改成"规则骨架 + 角色句"的结构判据（太松，把用户自己改过
   的指令也判成旧版内置，实测 6/6 真实编辑被丢弃）。**用户点了保存就是用户的意思，
   任何猜测都会误伤。** 代码里已无相关机制，测试里有断言防止它们复活。
3. 自定义指令是**单一覆盖槽，不分语言**：保存后中英两种界面读取 `GET /prompt?lang=xx`
   都返回同一份自定义文本、`isCustom` 都为 true。因此中文界面下存的中文自定义指令，
   在英文界面也会生效（这是既有设计，`lang` 只决定"未自定义时回落到哪一版内置默认"）。
   仅当提交的文本与**当前**内置默认（中或英任一）完全相同时，才视为"未自定义"并清掉覆盖。
